...
wait_healthy() {
  local svc="$1" name="$2" timeout="${3:-60}" t=0
  ...
  while true; do
    ...
    if (( t >= timeout )); then
      echo "❌ $name not healthy (got: $status)"
      docker compose ps
...
wait_healthy api devsecops-ai-platform-api 60
wait_healthy ai  devsecops-ai-platform-ai  60
